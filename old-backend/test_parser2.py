...
coefficients_5d = np.polyfit(
    df.dropna()["DateNumeric"], df.dropna()["5DayAvgProblemsSolved"], 1
)
...
sns.lineplot(
    data=df, x="Date", y="5DayAvgProblemsSolved", marker="o", label="5-Day Average"
)

# Add lines of best fit
plt.plot(
    df["Date"],
    polynomial_ps(df["DateNumeric"]),
    color="blue",
    linestyle="--",
    label="Best Fit: Problems Solved",
)
plt.plot(
    df["Date"],
    polynomial_5d(df["DateNumeric"]),
    color="orange",
    linestyle="--",
    label="Best Fit: 5-Day Avg",
)
...
sns.lineplot(data=df, x="Date", y="ProblemsSolved", marker="o", label="Problems Solved")
sns.lineplot(
    data=df, x="Date", y="5DayAvgProblemsSolved", marker="o", label="5-Day Avg"
)
...
for name, group in df.groupby("YearMonth"):
    ...
    plt.plot(
        group["Date"],
        poly(group["DateNumeric"]),
        linestyle="--",
        label=f"Best Fit: {name}",
    )
...
for name, group in df.groupby("Bimonthly"):
    ...
    plt.annotate(
        slope,
        xy=(midpoint_date, midpoint_value),
        xytext=(10, 5),
